  ...
  <section v-loading="pageLoading" style="height: 100%">
    <el-container style="height: 100%">
      <el-container>
        ...
        >
          <span style="float: left; font-size: 14px; line-height: 28px">{{
            document.form.label
          }}</span>
          <el-button
            v-if="checkPermission(['api:admin:document:updatecontent'])"
            type="primary"
            :disabled="!hasDocument"
            :loading="document.loadingSave"
            @click="save(false)"
            >保存文档</el-button
          >
        </el-header>
      ...
      >
        <el-container style="height: 100%; overflow: hidden">
            ...
            <div v-show="isDocTab">
              <el-button-group>
                ...
                <el-dropdown
                  v-if="
                    checkPermission([
                      'api:admin:document:addgroup',
                      'api:admin:document:addmenu',
                    ])
                  "
                >
                  ...
                  <template #dropdown>
                    ...
                    >
                      <el-dropdown-item
                        v-if="checkPermission(['api:admin:document:addgroup'])"
                        icon="el-icon-folder"
                        @click.native="onOpenAddGroup"
                        >新增分组</el-dropdown-item
                      >
                      <el-dropdown-item
                        v-if="checkPermission(['api:admin:document:addmenu'])"
                        icon="el-icon-tickets"
                        @click.native="onOpenAddMenu"
                        >新增菜单</el-dropdown-item
                      >
                    </el-dropdown-menu>
                  </template>
                </el-dropdown>
            ...
            <div v-show="isImgTab">
              <el-button-group>
                <el-button
                  type="primary"
                  :disabled="!hasDocument"
                  @click="getDocumentImages"
                  >刷新</el-button
                >
                <el-upload
                  v-if="
                    hasDocument &&
                    checkPermission(['api:admin:document:uploadimage'])
                  "
                  ref="upload"
                  :action="uploadUrl"
                  multiple
          ...
          >
                  ...
                  <template #default="{ $index, row }">
                    <el-button
                      v-if="
                        ...
                        (row.type === 2 &&
                          checkPermission(['api:admin:document:updatemenu']))
                      "
                      type="text"
                      icon="el-icon-edit"
            ...
            <div
              v-show="isImgTab"
              v-loading="document.loadingImageList"
              style="height: 100%"
            >
              <el-row :gutter="10" class="mc-upload-list">
                <el-col
                  v-for="img in document.images"
                  :key="img.uid ? img.uid : img.src"
                  :span="12"
                  class="mc-upload-list__item"
                >
                  <el-image
                    v-if="img.src"
                    :src="img.src"
                  ...
                  <span class="mc-upload-list__item-actions">
                    <span
                      class="cm-upload-list__item-preview"
                      @click="onOpenViewer(img.src)"
                      ><i class="el-icon-zoom-in"
                    /></span>
                    <span
                      v-if="checkPermission(['api:admin:document:deleteimage'])"
                      class="cm-upload-list__item-delete"
                      @click="onDeleteImage(img.src)"
    ...
    >
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button @click.native="documentGroup.visible = false"
            >取消</el-button
          >
          <my-confirm-button
            type="submit"
            :validate="validateGroup"
            :loading="documentGroup.loading"
            @click="onSubmitGroup"
          />
        </div>
      </template>
    ...
    <my-window
      ...
      "
      :title="(documentMenu.form.id ? '编辑' : '新增') + '文档'"
      :visible.sync="documentMenu.visible"
      @close="onCloseMenu"
    >
      <el-form
        ref="menuForm"
        :model="documentMenu.form"
        label-width="100px"
        :rules="formRules"
      >
        <el-form-item prop="parentIds" label="上级分组" width>
        ...
        <el-form-item label="描述" prop="description">
          <el-input
            v-model="documentMenu.form.description"
            auto-complete="off"
          />
        </el-form-item>
      </el-form>
      <template #footer>
        <div class="dialog-footer">
          <el-button @click.native="documentMenu.visible = false"
            >取消</el-button
          >
          <my-confirm-button
            type="submit"
            :validate="validateMenu"
            :loading="documentMenu.loading"
            @click="onSubmitMenu"
          />
        </div>
      </template>
...
export default {
  ...
  computed: {
    ...
    hasDocument() {
      return this.document.form.id != "";
    },
    imageIndex() {
      let previewIndex = 0;
      const srcIndex = this.document.images.findIndex(
        (a) => a.src === this.imageSrc
      );
      if (srcIndex >= 0) {
    ...
    uploadUrl() {
      return process.env.VUE_APP_BASE_API + "/admin/document/uploadimage";
    },
    imageUrls() {
  ...
  watch: {
    "document.form.content"(newVal, oldVal) {
      if (!this.document.form.id || this.document.form._first) {
      ...
      const me = this;
      this.document.timer = setTimeout(function () {
        me.save(true);
      }, 10000);
    },
